        ...
        <div className="fixed inset-0 flex items-center justify-center p-4">
          ...
          >
            <DialogPanel className="bg-white min-h-2/12 dark:bg-neutral-900 max-w-5xl w-full p-6 rounded-lg shadow-xl overflow-y-auto relative">
              <button
                onClick={onClose}
                className="absolute top-3 right-4 text-white text-2xl bg-gray-500 rounded-full p-2 w-6 h-6 flex items-center justify-center z-50"
              >
                ×
              </button>
                    ...
                    <div className="flex gap-2 mt-3 overflow-x-auto scrollbar-hide">
                      {project.images.map((src, index) => (
                        <button
                          ...
                          className={`w-16 h-12 border rounded-md overflow-hidden flex-shrink-0 ${
                            activeImage === index ? "border-blue-800" : "border-neutral-300"
                          }`}
                        >
                          <Image
                            src={src}
