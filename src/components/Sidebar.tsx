      ...
      {isLg && (
        <aside className="pt-18 w-150 h-screen fixed left-0 top-0">
          <div className="flex flex-col gap-4 h-full pl-10">
            ...
            <p className="text-sm text-gray-200 leading-snug mb-8 w-120">{t("sidebar_desc")}</p>
              ...
              {menuItems.map(item => (
                <li key={item.id}>
                  ...
                  >
                    {/* 왼쪽 네모 라인 */}
                    <span
                      className={`h-[0.15rem] group-hover:w-12 transition-all duration-300 rounded-full group-hover:bg-white ${
                        `#${item.id}` === hash ? "bg-white w-12" : "bg-gray-600 w-4"
                      }`}
                    />
                    {/* 메뉴 텍스트 */}
                    <span
                      className={`group-hover:text-white text-sm ${
                        `#${item.id}` === hash ? "text-white" : "text-gray-500"
                      }`}
                    >
                      {item.label}
                    </span>
